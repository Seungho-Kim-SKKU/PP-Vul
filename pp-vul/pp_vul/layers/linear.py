...
def fc_layer_converter_tmp(context: Context, C_in, layer):
    ...
    for o in range(CH_out):
        ...
        for i in range(CH_in):
            V_ker = [layer.weight.detach().tolist()[o][i]]
            Plaintext_ker = context.encoder.encode(V_ker, context.scale)

            context.evaluator.mod_switch_to_inplace(Plaintext_ker, C_in[i].parms_id())
            ciphertext = context.evaluator.multiply_plain(C_in[i], Plaintext_ker)
            context.evaluator.relinearize_inplace(ciphertext, context.relin_keys)
            context.evaluator.rescale_to_next_inplace(ciphertext)

            C_outs_each_channel.append(ciphertext)
        ciphertext = context.evaluator.add_many(C_outs_each_channel)
        ...
        C_outs.append(context.evaluator.rotate_vector(
            ciphertext, -1 * o, context.galois_key
        ))
                
    return context.evaluator.add_many(C_outs)