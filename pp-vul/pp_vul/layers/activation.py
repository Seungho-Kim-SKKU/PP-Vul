...
from pp_vul.utils.structure import Output
...
def cube(context: Context, In):
    ...
    for C in In:
        C_square = context.evaluator.square(C)
        context.evaluator.relinearize_inplace(C_square, context.relin_keys)
        context.evaluator.rescale_to_next_inplace(C_square)
        ...
        C_cube = context.evaluator.multiply(C, C_square)
        context.evaluator.relinearize_inplace(C_cube, context.relin_keys)
        context.evaluator.rescale_to_next_inplace(C_cube)
