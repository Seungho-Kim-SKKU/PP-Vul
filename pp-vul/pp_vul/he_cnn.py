...
class HE_CNN(torch.nn.Module):
    ...
    Conv2d | Flatten | FC | Sqaure | Approx ReLU
    """
    ...
    def calculate_depth(self):
        ...
        for layer_name, layer in self.model.named_children():
            ...
            elif layer.__class__.__name__ == 'Cube':
                req_depth += 2
    ...
    def forward(self, C_in: list, _time=False):
        ...
        Out = Output(C_in, self.Img)
        if _time:
            _order = 0
            CHECK_TIME = []
            CHECK_TIME.append(time.time())
            # print('%.3f' %(CHECK_TIME[_order] - START_TIME))
        for layer_name, layer in self.model.named_children():
            layer_params = getattr(self.model, layer_name)
            
            if layer_name == 'dropout' or layer_name == 'Flatten':
                continue
                ...
                if (layer_params.groups == 1):
                    ...
                    Out = conv2d_layer_converter_depthwise(
                        self.context, Out, self.Img, layer_params, self.data_size
                    )
            ...
            if _time:
                ...
                #     print('%s TIME\t %.3f sec' %(layer_name, CHECK_TIME[_order] - CHECK_TIME[_order - 1]))
                    ...
                    print('%.3f' %(CHECK_TIME[_order] - CHECK_TIME[_order - 1]))
